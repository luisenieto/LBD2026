...
-- WHERE nombre BETWEEN 'Desire' AND 'Joy'; -- between también se puede usar para rango de cadenas 
...
-- Conclusión: NULL no es igual a nada, ni siquiera a otro NULL

-- Ejecutar las siguientes 2 consultas juntas para poder comparar
SELECT idProducto, nombre, descripcion
FROM Productos
...
SELECT idProducto, nombre, descripcion
FROM Productos
WHERE descripcion <=> NULL; -- operador null-safe
...
SELECT MAX(idCargo) FROM `Cargos`;

SELECT MAX(idCargo) + 1 FROM `Cargos`;

SELECT COALESCE(MAX(idCargo), 0) + 1 FROM `Cargos`;


-- Consultas básicas - IFNULL

SELECT IFNULL(MAX(idCargo), 0) + 1 FROM `Cargos`;
...
-- Múltiples tablas – JOIN explícito vs sintaxis implícita
-- Ejecutar las 2 siguientes consultas juntas para poder comparar
-- JOIN explícito
SELECT idProducto, Productos.nombre, Marcas.nombre AS 'Marca', Categorias.nombre AS 'Categoría'
FROM Productos JOIN Categorias
ON Productos.idCategoria = Categorias.idCategoria
...
-- Ambas consultas generan el mismo plan de ejecución y tienen el mismo rendimiento
-- La consulta que emplea JOIN tiene menos riesgo de errores que la que no emplea JOIN (se puede olvidar una comparación en el WHERE)
-- La consulta que emplea JOIN soporta OUTER JOIN, mientras que la otra no (la sintaxis implícita es equivalente a un INNER JOIN) 

delete from AtributosDelProducto;
...
-- Mostrar el diagrama (tablas Productos, Marcas, Categorias, Atributos y AtributosDelProducto)
...
-- Mostrar el diagrama (tablas ProductosJSON, Marcas, Categorias)
...
-- Mostrar los productos que sean Televisión o Celulares
-- Ejecutar las siguientes 2 consultas juntas para poder comparar
SELECT idProducto, p.nombre as Producto
FROM Productos p JOIN Categorias c 
ON p.idCategoria = c.idCategoria
WHERE c.nombre IN ('Televisión', 'Celulares');

SELECT idProducto, p.nombre as Producto
from Productos p -- 1, 3, 4, 5
where p.idCategoria in (select idCategoria FROM Categorias where nombre in ('Televisión', 'Celulares'));


-- Mostrar sólo las marcas que tengan productos asociados
