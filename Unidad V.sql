...
-- Devuelve la cantidad de clientes
BEGIN  
	DECLARE vCantidad INT; -- insensible a mayúsculas/minúsculas

	SELECT COUNT(*) INTO vCantidad FROM Clientes;
	
	SET pCantidad = vCantidad;
END //
DELIMITER ;
...
DROP PROCEDURE IF EXISTS ListarClientesPorCompania;

DELIMITER //
CREATE PROCEDURE ListarClientesPorCompania(Compania VARCHAR(40))
BEGIN  
	SELECT * 
	FROM Clientes
    WHERE Compania = Compania; -- Compania es el nombre de la columna => siempre será verdadera la condición
END //
DELIMITER ;

CALL ListarClientesPorCompania('Alfreds Futterkiste'); -- todos los clientes
CALL ListarClientesPorCompania('Around the Horn'); -- todos los clientes
...
BEGIN  
	DECLARE Compania VARCHAR(50);

	SET Compania = 'Mi Compania';

	SELECT IDCliente, Compania, Nombre 
	FROM Clientes;
	-- En el select prevalen las variables sobre las columnas 
END //
DELIMITER ;

CALL ListarClientes(); -- para todos los clientes se muestra 'Mi Compania'


DROP PROCEDURE IF EXISTS ListarClientesPorCompania;

DELIMITER //
CREATE PROCEDURE ListarClientesPorCompania(pCompania VARCHAR(40))
BEGIN  
	DECLARE vCompania VARCHAR(50);

	SET vCompania = 'Mi Compania';

	SELECT IDCliente, Compania, vCompania, Nombre 
	FROM Clientes
    WHERE Compania = pCompania; -- Compania es el nombre de la columna => siempre será verdadera la condición
END //
DELIMITER ;

CALL ListarClientesPorCompania('Alfreds Futterkiste'); -- sólo 'Alfreds Futterkiste'
...
DELIMITER //
CREATE PROCEDURE AltaEstudiante(pApellidos VARCHAR(20), pNombres VARCHAR(45), pDNI CHAR(8), pCX CHAR(10), OUT mensaje VARCHAR(100))
...
SALIR: BEGIN  
	...
	IF (pApellidos IS NULL) OR (pNombres IS NULL) OR (pDNI IS NULL) OR (pCX IS NULL) THEN
		SET mensaje = 'Error en los datos del estudiante';
        LEAVE SALIR;
	ELSEIF EXISTS (SELECT * FROM Estudiantes WHERE dni = pDNI) THEN
		SET mensaje = 'Ya existe un estudiante con ese DNI';
        LEAVE SALIR;
	ELSEIF EXISTS (SELECT * FROM Estudiantes WHERE cx = pCX) THEN
		SET mensaje = 'Ya existe un estudiante con ese CX';
        LEAVE SALIR;
	ELSE
		SELECT COALESCE(MAX(idEstudiante), 0) + 1 INTO vIDEstudiante FROM Estudiantes;
	
		INSERT INTO Estudiantes VALUES (vIDEstudiante, pApellidos, pNombres, pDNI, pCX);
        SET mensaje = 'Estudiante creado con éxito';
    END IF;
END //
DELIMITER ;

CALL AltaEstudiante('Apellido1', 'Nombre1', '111', 'CX111', @resultado); -- funciona
SELECT @resultado; -- hay que revisar el valor del parámetro de salida
SELECT * FROM Estudiantes;

CALL AltaEstudiante('Apellido2', 'Nombre2', NULL, 'CX222', @resultado); -- no genera un error la llamada con estos valores
SELECT @resultado; -- hay que revisar el valor del parámetro de salida
SELECT * FROM Estudiantes;

CALL AltaEstudiante('Apellido2', 'Nombre2', '111', 'CX222', @resultado); -- no genera un error la llamada con estos valores
SELECT @resultado; -- hay que revisar el valor del parámetro de salida
SELECT * FROM Estudiantes;

DELETE FROM Estudiantes WHERE dni = '111';
...
DELIMITER //
CREATE PROCEDURE AltaEstudiante(pApellidos VARCHAR(20), pNombres VARCHAR(45), pDNI CHAR(8), pCX CHAR(10))
-- Crea un alumno siempre y cuando no haya otro con el mismo DNI y/o CX
BEGIN  
	DECLARE vIDEstudiante INT;
...
CALL AltaEstudiante('Apellido1', 'Nombre1', '111', 'CX111'); -- funciona
SELECT * FROM Estudiantes;

CALL AltaEstudiante('Apellido2', 'Nombre2', NULL, 'CX222'); -- genera el código SQLSTATE 45000
...
BEGIN  
	DECLARE vIDEstudiante INT;
	IF (pApellidos IS NULL) OR (pNombres IS NULL) OR (pDNI IS NULL) OR (pCX IS NULL) THEN
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Error en los datos del estudiante', MYSQL_ERRNO = 45000;
	ELSEIF EXISTS (SELECT * FROM Estudiantes WHERE dni = pDNI) THEN
		SIGNAL SQLSTATE '45001' SET MESSAGE_TEXT = 'Ya existe un estudiante con ese DNI', MYSQL_ERRNO = 45001;
	ELSEIF EXISTS (SELECT * FROM Estudiantes WHERE cx = pCX) THEN
		SIGNAL SQLSTATE '45002' SET MESSAGE_TEXT = 'Ya existe un estudiante con ese CX', MYSQL_ERRNO = 45002;
	ELSE
		SELECT COALESCE(MAX(idEstudiante), 0) + 1 INTO vIDEstudiante FROM Estudiantes;
...
CALL AltaEstudiante('Apellido2', 'Nombre2', NULL, 'CX222'); -- genera el código SQLSTATE 45000 y el mensaje correspondiente
...
BEGIN  
	...
	IF (pApellidos IS NULL) OR (pNombres IS NULL) OR (pDNI IS NULL) OR (pCX IS NULL) THEN
		SIGNAL datos_invalidos SET MESSAGE_TEXT = 'Error en los datos del estudiante', MYSQL_ERRNO = 45000;
	ELSEIF EXISTS (SELECT * FROM Estudiantes WHERE dni = pDNI) THEN
		SIGNAL dni_repetido SET MESSAGE_TEXT = 'Ya existe un estudiante con ese DNI', MYSQL_ERRNO = 45001;
	ELSEIF EXISTS (SELECT * FROM Estudiantes WHERE cx = pCX) THEN
		SIGNAL cx_repetido SET MESSAGE_TEXT = 'Ya existe un estudiante con ese CX', MYSQL_ERRNO = 45002;
	ELSE
		SELECT COALESCE(MAX(idEstudiante), 0) + 1 INTO vIDEstudiante FROM Estudiantes;
...
INSERT INTO Trabajos VALUES(74, 'Sistema automático de matrículas', 'SW', 6, '2019-03-15', '2019-03-28', NULL);
